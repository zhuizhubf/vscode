...
export function getAccessibilityHelpText(keybindingService: IKeybindingService): string {
	...
	content.push(descriptionForCommand('interactiveSession.action.focus', localize('interactiveSession.action.focus', 'The GitHub Copilot: Focus on GitHub Copilot View ({0}) command focuses the chat request/response list, which can be navigated with UpArrow/DownArrow.',), localize('interactiveSession.action.focusNoKb', 'The Focus Interactive Session command focuses the chat request/response list, which can be navigated with UpArrow/DownArrow and is currently not triggerable by a keybinding.'), keybindingService));
