...
export class ViewZoneAlignment extends Disposable {
	...
	constructor(
		...
		const alignmentsSyncedMovedText = derived<IRangeAlignment[] | null>('alignments', (reader) => {
			...
			const mappings = syncedMovedText.changes.map(c => new DiffMapping(c, undefined, undefined));
		...
		this._register(autorun('update', reader => {
			...
			if (m) {
				const trueTopOriginal = this._originalEditor.getTopForLineNumber(m.lineRangeMapping.originalRange.startLineNumber) - this._origExtraHeight.get();
				const trueTopModified = this._modifiedEditor.getTopForLineNumber(m.lineRangeMapping.modifiedRange.startLineNumber) - this._modExtraHeight.get();
	...
	/*alignLines(originalLineNumber: number, modifiedLineNumber: number, fix: 'original' | 'modified') {

		//scrollTopDelta: number
		const currentDelta = this._modExtraHeight.get();
		const curScrollTop = this._originalEditor.getScrollTop();
		this._modExtraHeight.set(scrollTopDelta, undefined);

		const originalEditorViewZoneHeightBefore = currentDelta < 0 ? -currentDelta : 0;
		const modifiedEditorViewZoneHeightBefore = currentDelta > 0 ? currentDelta : 0;
		const originalEditorViewZoneHeightAfter = scrollTopDelta < 0 ? -scrollTopDelta : 0;
		const modifiedEditorViewZoneHeightAfter = scrollTopDelta > 0 ? scrollTopDelta : 0;

		this._originalEditor.setScrollTop(curScrollTop + (originalEditorViewZoneHeightAfter - originalEditorViewZoneHeightBefore));


		const val = this._modifiedScrollOffset.get();
		this._modifiedScrollOffset.set(val - scrollTopDelta, undefined, false);

		this._modifiedScrollOffset.set(val, undefined, true);
	}*/
}
